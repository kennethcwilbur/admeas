...
## This Deck's Value Proposition {.medium}

- One day soon, someone may ask, or tell, you how well the ads worked
  - Advertising measurement is a hard, important, interesting problem without any general solution
...
## More Recent Online Ad Revenues

:::: {.columns}

::: {.column width="54%"}
![](AMimages/IARRgrowthbyformat2021-25-cropped.png){fig-align="center" height="370px"}
:::

::: {.column width="46%"}
![](AMimages/emarketer_us_ad_spend_by_company_2019_2025.jpg){fig-align="center" height="370px"}
:::

::::

::: {.callout-note appearance="minimal"}
These data expand the Internet bar. The Interactive Advertising Bureau (IAB) collects ad sales revenue by format from online ad sellers and supply chain firms. Total spending grew from $179.4B in 2021 to $282.2B in 2025, 0.92% of gross domestic product (GDP). What else do you see?
:::

::: {.source-url}
Left: [IAB (2026)](https://www.iab.com/wp-content/uploads/2026/04/IAB_PwC_Internet_Ad_Revenue_Report_Full_Year_2025_April_2026.pdf){target="_blank"}. Right: [EMARKETER (Dec 2025), via Hanzlik (2026)](https://www.linkedin.com/posts/vladhanzlik_many-people-assume-amazon-google-and-meta-activity-7439279754484183041-cN9B){target="_blank"}
:::
...
## Ad Sales Revenue as % of Global GDP, 2011-2025 {.callout-sm}

![](AMimages/bigsellers_adshare_world_2011_2025.png){fig-align="center" height="350px"}
...
::: {.fig-note}
The numerator is advertising revenue only, not total company revenue. Each firm reports it as a separate line in its 10-K revenue note — Alphabet's "Google advertising" (Search & other, YouTube ads, Google Network), Meta's "Advertising," Amazon's "Advertising services," and Microsoft's "Search and news advertising." In 2025 those lines were 73%, 98%, 10% and 5% of each firm's total revenue, so the chart is not a ranking of company size. The denominator is global nominal GDP. Amazon's line starts at its first disclosure (2019); Microsoft is plotted at June fiscal years; its FY2014–15 points are inferred from growth rates disclosed in those years' 10-Ks, which reported no levels.
:::
::::

::: {.source-url}
Data: company 10-K filings; [World Bank WDI](https://data.worldbank.org/indicator/NY.GDP.MKTP.CD?locations=1W){target="_blank"}. Chart concept: [Marto and Le (2024)](https://www.stlouisfed.org/on-the-economy/2024/oct/rise-digital-advertising-economic-implications){target="_blank"}
:::
...
- Suppose we pay \$10 to buy 1,000 digital ad Opportunities to See ("OTS"). Suppose 3 people click, 1 person buys.
- Ad profit > 0 if transaction margin > \$10
    - But we bought ads for 999 people who didn't buy
- Or, ad profit > 0 if customer lifetime value (CLV) > \$10
...
::: {.callout-note appearance="minimal"}
Google Performance Max (Pmax) is the ultimate expression of programmatic advertising. You give Google your goals, your budget, and things it can say in ads. Google decides where, when and how to spend your money, designs your ad, then tells you how well it did. Launched in 2021; over 1 million advertisers served by 2025. Meta's Advantage+ is similar.
:::
...
::: {.callout-note appearance="minimal"}
2025Q2 data show that DSP takes 11%, SSP takes 15%, publisher receives 75%, and about half of that is verifiably viewable by human recipients. What are demand-side platforms (DSP), supply-side platforms (SSP), invalid traffic (IVT), Measurable, Viewable, and [made-for-advertising](https://deepsee.io/blog/2-tales-one-site-how-arbitrage-sites-manipulate-metrics){target="_blank"} (MFA) sites?
:::
...
## Attention & Ad Cost by Medium {.tight-media}

![](AMimages/attention_vs_cpm_2025.png){fig-align="center" height="398px"}

:::: {.columns}

::: {.column width="72%"}
::: {.callout-note appearance="minimal"}
Advertising media vary in average attention attracted (i.e. eyes-on-screen) and advertising price. How does average attentiveness correlate with ad price across media? Why?
:::
:::
...
::: {.source-url}
Attention: [Lumen Research](https://www.lumen-research.com/){target="_blank"} 2025 channel norms | Price: [Adsposure (2026)](https://www.adsposure.com/cpm/){target="_blank"}; cinema [AdQuick](https://www.adquick.com/movie-theater-cinema-advertising){target="_blank"}; gaming [PubScale](https://pubscale.com/blog/in-game-advertising-guide){target="_blank"} | Audio formats excluded
:::
...
The average US corporation spends about 3.1% of gross profit on advertising deductions. Gross profit usually ranges from 3-5x net profit, so the modal firm could increase net profit by 10.3-18.5% by setting ads to zero (i.e. 100/(100-9.3) to 100/(100-15.5)). Or could it? What would happen to top line revenue and cost efficiencies? [2 missing data points were withheld by source for confidentiality purposes]
:::
...
- Commuters carrying umbrellas followed by rain later in the day
    - Forward-looking behavior: Rooster noises do not cause sunrise
...
::: {.callout-note appearance="minimal"}
This A/B test triggered a "revenue too high" alert at Microsoft Bing in 2012. The treatment improved horizontal space usage, enlarged a selling argument, and expanded the clickable surface in search ads. It increased revenue 12%—over \$100 million per year—without harming user experience metrics.
:::
...
![](AMimages/4typesofanalytics-trimmed.png){fig-align="center" height="394px"}
...
![](AMimages/SERP_usedguitarlespaul-trimmed.png){fig-align="center" height="415px"}

::: {.callout-note appearance="minimal"}
eBay taught us that correlational advertising measurement is questionable, and that firms should use experiments to measure causal advertising effects. However, most companies were not ready for that message yet. This 2026 screenshot shows that eBay lost its organic search engine results page (SERP) real estate and started buying Google search ads again. As it should do when ads are profitable.
:::
...
![](AMimages/2024admeasurementtrends.png){fig-align="center" height="432px"}

::: {.callout-note appearance="minimal"}
Incrementality & marketing mix models (MMM) were trends #1 & #2; the only other trend was e-commerce metric proliferation.
:::
...
## Ad Experiments in Google Trends & Buy-side Surveys

:::: {.columns}

::: {.column width="45%"}
![](AMimages/gtrends_adexp.png){fig-align="center" height="294px"}
:::

::: {.column width="55%"}
![](AMimages/iab admeas survey 2026.png){fig-align="center" height="294px"}
:::

::::

::: {.callout-note appearance="minimal"}
We're a few years into a generational shift. Smaller, independent ad agencies are making the most noise about incrementality. However, the correlation between advertising and sales (corr(ad,sales)) is not going away. Union(correlations, experiments) should exceed either alone.
:::
...
- We want to maximize profits $\Pi = \Sigma_i \pi_i(Y_i(T_i), T_i)$, where $\pi_i$ is person $i$'s profit contribution
...
3. **Use correlations:** Assume past treatments were assigned randomly, use past data to estimate $\frac{\partial Y_i}{\partial T_i}$
    ...
    - Are we paying our demand-side platforms (DSPs) to distribute our ads randomly?
...
- *Advertising measurement* quantifies ad delivery, exposure and outcomes to improve advertising efforts
    - We focus on sales/conversions, as they matter most
    - Brand campaign measurement usually focuses on cost and intermediate metrics such as attention and brand attitudes. General principles include independence and transparency in measurement; these must be checked, as they cannot be assumed
...
We also should measure delivery and funnel-wide key performance indicators (KPIs), e.g. brand metrics, visits, add-to-cart, sales, revenue, ...

- We usually get economies of scope in measurement

::: {.callout-note appearance="minimal"}
Brand Lift Tests measure ad effects on brand attitude surveys, but these are often underpowered. CLV/CAC (customer acquisition cost) is increasingly common within subscription businesses
:::

## Diminishing Returns

![](AMimages/roas_vs_mroas.png){fig-align="center" height="350px"}

::: {.callout-note appearance="minimal"}
In theory, we buy the best ad opportunities first, so increasing spend should lower marginal returns ("saturation"). Marginal ROAS (mROAS) is the tangent to the curve. Nonlinearity means ROAS ≠ mROAS. We use ROAS for overall evaluation, and mROAS for budget reallocation. The common adage to "max your ROI" usually leaves money on the table. (Why?) An optimal budget allocation equalizes mROAS across channels. (Why?) What level of ad spending actually maximizes mROAS?
:::
...
- A "media mix model" (mMM) relates sales to ads/marketing communications (marcom) channels
...
- Outputs include ad elasticities, correlational mROAS estimates, counterfactual budget reallocations
...
::: {.callout-note appearance="minimal"}
Famous books present descriptive evidence about how brands have grown, then extrapolate to prescriptive "laws" about how marketers should act. Parsimonious advice can be appealing, but has been called pseudo-science
:::
...
# Causal Advertising Measurement {background-image="AMimages/xkcd_stats.png" background-size="386px 156px" background-position="right 45px bottom 10px"}
...
## Popular ad experiments {.medium}

1. Going-dark design. Randomly withhold ads. Estimates iROAS

2. Weight test: Randomize spending between high-spend and low-spend treatments. Estimates incremental mROAS

3. Randomize messages within a campaign
    - Nominally free; Mine competitor messages in [ad libraries](https://indicator.media/p/a-guide-to-investigating-digital){target="_blank"} for ideas
    - Advertising pros often believe that creative messages are first-order drivers of ad effects (e.g., [Circana 2023](https://www.circana.com/post/brand-loyalty-impact-on-sales){target="_blank"}, [Kantar 2024](https://www.kantar.com/north-america/inspiration/advertising-media/what-do-the-most-creative-and-effective-digital-and-tv-ads-do-differently){target="_blank"}, [Magna+Yahoo 2025](https://ipg-wp-media-mgl-glb.s3.us-east-2.amazonaws.com/magna/wp-content/uploads/2023/04/27162350/MAGNA-Yahoo-Creative-the-Performance-Powerhouse.pdf){target="_blank"})

4. Randomize budget across platforms, publishers, behavioral targets, contexts

::: {.callout-note appearance="minimal"}
Geo-experiments randomize treatments across places (e.g., cities) and time periods. Consumer-level experiments usually randomize treatments across devices, accounts or cookies. Geo-Experiments have been around forever, but often return inconclusive results. Consumer-level randomization became widespread with the internet, but it too may yield inconclusive results. [Lewis and Rao (2015)](https://drive.google.com/file/d/12TZb4l40abpHoD0kU_tbDm-j1qFjmOD7/view?usp=sharing){target="_blank"} taught us that the challenge in both cases involves low response rates and noisy environments
:::
...
## Different Designs Estimate Different Things

::: {style="margin: -12px 0 0 0; text-align: center;"}
![](AMimages/estimand_goingdark.png){height="320px" style="margin: 0 2px;"} ![](AMimages/estimand_weighttest.png){height="320px" style="margin: 0 2px;"} ![](AMimages/estimand_multicell.png){height="320px" style="margin: 0 2px;"}
:::

::: {.callout-note appearance="minimal" style="margin-top: -2px; font-size: 0.68em;"}
Going-dark design estimates iROAS: the causal effect of all observed advertising vs. zero advertising. It quantifies campaign performance overall, but cannot say whether spending level was optimal<br>Weight-test design estimates (approximate) incremental mROAS: can inform spending decisions, but does not focus on overall profitability<br>Multi-cell weight tests can estimate both iROAS and incremental mROAS, but spread sample across more treatments<br>The challenge is to estimate both the height and the curvature of the response curve<br>Recall that MMM can estimate correlational mROAS; incremental mROAS is the causal counterpart
:::

::: {.source-url style="font-size: 0.50em;"}
[Waisman and Gordon (2025)](https://arxiv.org/pdf/2302.13857){target="_blank"}
:::
...
- Can we agree on the opportunity cost of the experiment? "Priors"
    - Confident execs argue control-condition exposures reduce profits due to foregone advertisements
- How will we act on the (uncertain) findings? Have to decide before we design. We don't want "science fair projects"
    - Simple example: Suppose we estimate iROAS at 1.5 with confidence interval (c.i.) [1.45, 1.55]. Or, suppose we estimate iROAS at 1.5 with c.i. [-1.1, 4.1]. What actions would follow each?
...
## Problems with Ad Experiments {.smaller}

1. Low power, noisy estimates -> limited decision guidance
    - Sales volatility often swamps true ad effects ([Lewis & Rao 2015](https://gwern.net/doc/economics/advertising/2015-lewis.pdf){target="_blank"})
    - [Feit & Berman (2019)](https://arxiv.org/abs/1811.00457){target="_blank"} choose test size to maximize expected profit
2. Low external validity / generalizability, transportability
    - A finding depends heavily on its context, and may change dramatically when the context changes ([Shapiro, Hitsch & Tuchman 2021](https://drive.google.com/file/d/1f3JLTvIsltKej0DDae_vkZyAOmBXI13Y/view?usp=drive_link){target="_blank"})
3. No true control
    - If an experiment changes ads, and ads change demand, then demand may change optimal recommendations, prices, reviews, inventory, competitor actions or other market signals, which then affect both treatment and control cells ([Sahni 2016](https://journals.sagepub.com/doi/10.1509/jmr.14.0274){target="_blank"}, [Blake & Coey 2014](https://dominiccoey.github.io/assets/papers/marketplace_experiments.pdf){target="_blank"}, [Du et al. 2019](https://drive.google.com/file/d/1V9TYcKEw3RSxytTkk8W7RIM7p2uL-WD9/view?usp=drive_link){target="_blank"})
    - [Identity fragmentation](https://www.mile.tech/blog/identity-matching-adtech){target="_blank"} means a user may be in both cells on different devices ([Lin & Misra 2022](https://arxiv.org/abs/2008.12849){target="_blank"})
    - In going-dark designs, carryover effects from pre-experimental campaigns can bias control-cell sales upwards

::: {.callout-note appearance="minimal"}
[Johnson (2023)](https://papers.ssrn.com/sol3/papers.cfm?abstract_id=3581396){target="_blank"} reviews best practices for digital advertising experiments.
:::

## Problems with Ad Experiments {.medium}

4. Silent failure of other conditions
    - Ad fraud, ad blocking, non-visibility & non-attention: All cause noncompliance & distort analyses ([Johnson 2023](https://papers.ssrn.com/sol3/papers.cfm?abstract_id=3581396){target="_blank"})
    - Divergent delivery ([Braun & Schwartz 2025](https://drive.google.com/file/d/11BhiE3wJqaKQ9S73Mjt1hbZJRhkvZvv5/view?usp=drive_link){target="_blank"})
5. Costs & biases: Large experiments are too costly to run continuously
    - Opportunity cost of nontargeted consumers ([Ghost ads](https://drive.google.com/file/d/1V9_YHaeiwZkIp4zE3bHhvfRfa-4KrOmT/view?usp=drive_link){target="_blank"} as potential remedy)
    - Time delay to decision relevance
    - Experiments may bias us toward short-term effects of performance ads on certain platforms
    - Platforms often intervene to maximize total ad-linked conversions, rather than maximizing the incremental effect of ads on conversions ([Haus 2025](https://www.haus.io/blog/what-youre-actually-measuring-in-a-platform-a-b-test){target="_blank"}, [Braun & Schwartz 2025](https://drive.google.com/file/d/11BhiE3wJqaKQ9S73Mjt1hbZJRhkvZvv5/view?usp=drive_link){target="_blank"})
...
::: {.callout-note appearance="minimal"}
I made this graph showing DraftKings and FanDuel branded keyword search volume from 9:01-9:59pm ET during the 2015 National Football League (NFL) season opener. TV ads increased search volume by 15-25x, with positive competitive spillovers, and effects that returned to baseline within 5 minutes. Commercial minutes are shaded, showing the spikes came from ads, not just commercial breaks.
:::
...
- E & Q-E should be "yes-and-when-valid," not "either-or"

::: {.source-url}
[Imbens and Xu (2025)](https://pubs.aeaweb.org/doi/pdf/10.1257/jep.20251440){target="_blank"} cover vitally important Q-E credibility checks
:::
...
::: {.callout-note appearance="minimal"}
Management challenge: What do we do when Attribution says ROAS was 5.9, MMM says ROAS was 2.3 +/- 1.2 and Experiment says iROAS was 0.6 +/- 2.1?
:::
...
- [Gordon, Moakler & Zettelmeyer (2026)](https://arxiv.org/abs/2304.06828){target="_blank"} propose Predicted Incrementality by Experimentation (PIE) in which (1) past campaigns with experiments are used to estimate how campaign features predict incremental ad effects, then (2) estimates are used to predict untested campaigns' incrementality. The model generalized well to nontraining campaigns. [PyMC-Marketing](https://pypi.org/project/pymc-marketing/){target="_blank"} includes an open-source implementation

::: {.callout-note appearance="minimal"}
PIE stability should be tested in each instance, not assumed
:::
...
::: {.callout-note appearance="minimal"}
All integration approaches can be used simultaneously
:::

## Frontier Topics {.medium}

- Optimal Brand Advertising Measurement: How to maximize loyalty, minimize price response
- Incrementality in Time: Can we distinguish purely incremental purchases from time-accelerated purchases that would have occurred later?
    - Requires counterfactual predictions of *future* events ([Lambrecht, Tucker & Zhang 2024](https://lbsresearch.london.edu/id/eprint/2886/7/lambrecht-et-al-2023-tv-advertising-and-online-sales-a-case-study-of-intertemporal-substitution-effects-for-an-online.pdf){target="_blank"})
- Incrementality $\ne$ Optimality: Incrementality asks whether the ads truly worked; optimality asks whether the ads maximized profit
    - How do we learn profit-maximizing combinations of Message, Target, Media, Context, Ad Price, Ad Frequency?
- How best to integrate quasi-experimental analyses
- How to choose experiments to maximize learning value ([Epanomeritakis & Viviano 2026](https://arxiv.org/pdf/2510.23434){target="_blank"})

::: {.callout-note appearance="minimal"}
Optimality should be the next frontier after incrementality is better established and integrated
:::

## Frontier: Causal Brand Ad Measurement {.smaller}

- Brand ads are designed to work over a long time horizon, but maintaining a long-term control condition is very expensive and often impossible
    - May violate experimental necessary conditions, e.g., SUTVA
    - Most marketers test brand ads before they run, rely on correlational brand ad measurement, and/or don't causally measure brand ads
- Most promising imho: Test how brand ad spend changes causal performance ad effects
    - Performance ads can be hard to scale due to increasing CAC, and 'fishing out the pond'
    - Using upper-funnel ads to 'warm up' the target predicts a brand $\times$ performance interaction
    - Brand ads tend to be beautiful and clear; performance ads often look cheap or tacky; performance ads alone might be a local maximum
    - Some sophisticated companies started as performance-only & later became huge brand advertisers; how do you think they justify it internally?

![](AMimages/brand-opens-door-2panel-vertical.svg){.absolute bottom="10" right="10" height="190px"}

::: {.callout-note appearance="minimal" style="width: 78%;"}
Ad measurement debates often confuse absence of evidence with evidence of absence. As a performance marketer, it can be uncomfortable to acknowledge that we cannot reasonably estimate causal brand ad effects. But does that mean the effects are zero? Can we know that?
:::

::: {.source-url}
[Laads (2015)](https://www.laadsmarketing.com/the-man-in-the-chairyesterday-and-today/){target="_blank"}
:::
...
- Takeaways, Further Study, AdMeas the Game, Thanks to
...
- Joel Barajas (Walmart), Rick Bruner (Central Control), Peter Daboll (DAIVID), Tom Flanagan (Flywire), Mike Follett (Lumen), Carl Mela (Duke), Prabhath Nanisetty (Snowflake), and Koen Pauwels (Northeastern) for helpful comments
